...
// ── Hardened custom overlay creation ───────────────────────────────────────
static HWND CreateCustomOverlay(HINSTANCE hInst, int x, int y, int w, int h) {
    ...
    std::wstring className = systemClasses[std::chrono::steady_clock::now().time_since_epoch().count() % 4];
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, int) {
    ...
    while (!mem.Init(xorstr_(L"cs2.exe")))
        Sleep(1000);
    ...
    g_overlay = CreateCustomOverlay(hInst, gr.left, gr.top, g_width, g_height);
        ...
        if (localController && localPawn) {
            ...
            // m_bSpottedByMask uses slot = handle & 0x1FF as the bit index.
            {
                ...
                if (localPawnHandle && localPawnHandle != 0xFFFFFFFF)
                    g_localEntityIndex = static_cast<int>(localPawnHandle & 0x1FF);
            }
